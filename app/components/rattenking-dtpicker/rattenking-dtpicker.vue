<template>
	<picker style="background:white;border-radius: 20upx;border:1upx solid #e0e0e0;" class='rui-picker rui-class' mode="multiSelector" :range="times" :value="timesIndex" :disabled="curDisabled" @change='changeDate' @cancel="cancelDate" @columnchange="columnchangeDate">
	  {{curValue}}
	</picker>
</template>
...
<script>
	...
	export default {
		...
		watch: {
			...
			curValue(val) {
				this.curValue = val;
				this.$emit('change', val,this.form_i);
			},
			times(val){
				this.times = val;
			},
