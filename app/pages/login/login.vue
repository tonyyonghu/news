			...
			<form @submit="formSubmit">
				<view>
					<view class="uni-form-item uni-column">
						<input class="uni-input"  placeholder="请输入账号" name="user_string" type="text" />
					</view>
				</view>
				<view>
					<view class="uni-form-item uni-column">
	...
	export default {
		...
		methods: {
			formSubmit: function(e) {
				...
				if (e.detail.value.user_string == '' || e.detail.value.password == '') {
					if (e.detail.value.user_string == '') {
						var content = '请输入账号';
					} else {
						var content = '请输入密码';
					}
					...
					server.postJSON('/user/login', e.detail.value, function(res) {
						...
						if (res.statusCode == 200) {
							...
							that.globalData.userInfo = res.data.message.user;
